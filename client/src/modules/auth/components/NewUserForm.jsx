...
const NewUserForm = ({ isAdmin = false, customStyles = {}, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Usuario creado con éxito', { duration: 5000, position: 'top-left' })
      if (onSuccess) onSuccess()
    } catch (error) {
      const errorMessage = error.response?.data?.message || 'Error al crear el usuario'
      toast.error(errorMessage, { duration: 5000, position: 'top-left' })
    }
  }
