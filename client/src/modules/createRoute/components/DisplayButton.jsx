...
const DisplayAsideButton = ({ setter, functionSet, position, direction }) => {
  return (
    <div className={`absolute ${position} transform z-[500]`}>
      <button
        ...
        onClick={() => functionSet(!setter)}
      >
        <IoMdArrowDropleft
          size={30}
          className={`text-gray-700 transition-transform duration-300 ${getDirectionArrow(direction, setter)}`}
        />
      </button>
    </div>
  )
